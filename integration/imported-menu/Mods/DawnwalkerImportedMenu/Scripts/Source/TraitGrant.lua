...
cyberfox1337x.signature("imported_TraitGrant")
...
function M.Init(menu,getPlayer,log)
    ...
    -- Returns whether the index landed, so the automatic session read can be retried
    -- while the subsystems a fresh pawn depends on are still resolving.
    ...
    -- The in-game menu indexed the perks on open; the desktop menu never opens it, so
    -- with a save loaded the list stayed on its placeholder until Refresh was pressed
    -- (and Grant then failed with "Select a perk first"). Index once per session as
    -- soon as the player exists; an early failure propagates so the runtime retries.
    function M.SessionReady()
        -- Probe silently first: a retryable failure must not rewrite the status label,
        -- since an unrequested read that republishes state on every tick is noise.
        assert(valid(getPlayer(true)),'Perk index waits for the player')
        assert(pcall(function() snapshot(subsystem('CharacterDevelopmentSubsystem')) end),'Perk index not ready yet')
        assert(refreshList(),'Perk index failed')
    end
    ...
    local function grantSelected()
        ...
        ExecuteInGameThread(function()
            ...
            local ok,err=pcall(function()
                ...
                ExecuteWithDelay(1000,function() ExecuteInGameThread(function()
                    local checked,message=pcall(function()
                        ...
                        local granted=string.format('%s granted at rank %d/%d. No skill points were spent.',before.names[selectedId],target,before.max[selectedId])
                        refreshList()
                        -- refreshList publishes its own count line; the grant result is what the
                        -- player asked about, so it is restored after the list is rebuilt.
                        status(granted)
                    end)
                    if not checked then
            ...
            if not ok then
                ...
                status(mutationStarted and 'Perk grant failed after mutation began. Do not save; reload an earlier save. See UE4SS.log.' or (tostring(err):gsub('^.-%.lua:%d+: ','')))
            end
        end)
    end
