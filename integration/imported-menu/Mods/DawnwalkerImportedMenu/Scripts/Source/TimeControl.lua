...
cyberfox1337x.signature("imported_TimeControl")
...
function M.Register(menu)
    ...
    function M.ResetSession()
        busy=false
        -- An issued-but-unverified time change remains blocked until restart.
        if used then status("STOP: interrupted time verification. Reload the protected save and restart.") end
    end
