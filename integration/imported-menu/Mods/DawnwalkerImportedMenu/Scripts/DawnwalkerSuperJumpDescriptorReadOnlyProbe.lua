...
local function collect_read_only_evidence(context, descriptor_record)
    ...
    local output = {}
    local value_ok, asc_value = pcall(function()
        return context.ability_system:GetGameplayAttributeValue(descriptor_record.descriptor, output)
    end)
    if not value_ok then return nil, "GetGameplayAttributeValue rejected the borrowed descriptor: " .. tostring(asc_value) end
    local found = output.bFound
    if not is_finite_number(asc_value) or not nearly_equal(asc_value, current_value) then
        return nil, "the ASC descriptor readback does not match JumpVelocity.CurrentValue"
    end
    if found ~= true then return nil, "GetGameplayAttributeValue did not confirm JumpVelocity in the bFound out parameter" end
