...
cyberfox1337x.signature("imported_InfamyControl")
...
function M.Register(menu)
    ...
    function M.ResetSession()
        busy=false
        -- Keep the original post-mutation failure latch; only reset queued work.
        if failed then status("STOP: prior Infamy mutation is unverified. Reload the protected save and restart.") end
    end
