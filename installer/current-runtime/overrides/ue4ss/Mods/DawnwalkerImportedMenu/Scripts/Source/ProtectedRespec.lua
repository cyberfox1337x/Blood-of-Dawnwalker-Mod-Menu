...
cyberfox1337x.signature("imported_ProtectedRespec")
...
function M.New(resolve,schedule,log,status)
    ...
    local api={}
    function api.ResetSession()
        if running then tainted=true end
        running=false;armed=nil
        if tainted then status('STOP: interrupted or unverified respec. Reload the pre-respec save and restart.') end
    end
    function api.Cancel()
