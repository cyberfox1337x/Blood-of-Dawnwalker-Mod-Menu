...
export type DsavChunk = Readonly<{ encoded: Buffer; decodedBytes: number }>;
export type DsavContainer = Readonly<{
  header: Buffer;
  namesOffset: number;
  nodesOffset: number;
  payloadBytes: number;
  chunks: readonly DsavChunk[];
}>;
export type DsavNode = Readonly<{
  index: number;
  name: string;
  nextIndex: number;
  childIndex: number;
...
export type DsavDocument = Readonly<{
  container: DsavContainer;
  payload: Buffer;
  names: readonly string[];
  nodes: readonly DsavNode[];
}>;
export type DsavCodec = Readonly<{
  decompress: (chunks: readonly DsavChunk[]) => Promise<readonly Buffer[]>;
  compress: (chunks: readonly Buffer[]) => Promise<readonly DsavChunk[]>;
}>;

function requireFormat(condition: boolean, reason: string): asserts condition {
  if (!condition) throw new Error(`Unsupported save: ${reason}.`);
}

function hasMarker(bytes: Buffer, offset: number, marker: string): boolean {
  return offset >= 0 && offset + marker.length <= bytes.length && bytes.toString("ascii", offset, offset + marker.length) === marker;
}

export function parseDsavContainer(bytes: Buffer): DsavContainer {
...
export function encodeDsavContainer(container: DsavContainer, chunks = container.chunks): Buffer {
...
function parseNames(payload: Buffer, container: DsavContainer): readonly string[] {
...
export function parseDsavPayload(container: DsavContainer, payload: Buffer): DsavDocument {
  ...
  for (const node of nodes) {
    ...
    if (node.nextIndex !== 65535) requireFormat(nodes[node.nextIndex].offset >= node.offset + node.size, "next node overlaps its predecessor");
  }
  requireFormat(nodes[0].nextIndex === 65535, "root cannot have a sibling");
  const visited = new Set([0]);
  const pending = [nodes[0]];
  while (pending.length) {
    const parent = pending.pop()!;
    let childIndex = parent.childIndex;
    while (childIndex !== 65535) {
      const child = nodes[childIndex];
      requireFormat(!visited.has(childIndex), "node has more than one parent");
      requireFormat(child.offset > parent.offset && child.offset + child.size <= parent.offset + parent.size, "sibling exceeds its parent");
      visited.add(childIndex);
      pending.push(child);
      childIndex = child.nextIndex;
    }
  }
  requireFormat(visited.size === nodes.length, "node directory contains unreachable records");
  return { container, payload, names, nodes };
}

export async function decodeDsav(bytes: Buffer, codec: DsavCodec): Promise<DsavDocument> {
...
export async function encodeDsav(document: DsavDocument, payload: Buffer, codec: DsavCodec): Promise<Buffer> {
  ...
  if (!changedIndexes.length) return encodeDsavContainer(document.container);
...
// A structural edit changes the payload length, so the whole payload is re-chunked at
// the game's 128 KiB chunk size and the header's NAME / DWNT / end offsets are rewritten
// (the same shape the game produces for a fresh save). Every chunk is recompressed.
export async function encodeResizedDsav(
  document: DsavDocument,
  payload: Buffer,
  namesOffset: number,
  nodesOffset: number,
  codec: DsavCodec,
): Promise<Buffer> {
  requireFormat(payload.length > 0 && payload.length <= MAX_PAYLOAD_BYTES, "rebuilt payload size is outside supported bounds");
  requireFormat(DATA_START < namesOffset && namesOffset < nodesOffset && nodesOffset < payload.length + DATA_START, "rebuilt NAME/DWNT offsets are out of order");
  const header = Buffer.from(document.container.header);
  header.writeUInt32LE(namesOffset, 16);
  header.writeUInt32LE(nodesOffset, 20);
  header.writeUInt32LE(payload.length + DATA_START, 24);
  const pieces: Buffer[] = [];
  for (let cursor = 0; cursor < payload.length; cursor += MAX_DSAV_CHUNK_BYTES) pieces.push(payload.subarray(cursor, Math.min(payload.length, cursor + MAX_DSAV_CHUNK_BYTES)));
  const compressed = await codec.compress(pieces);
  requireFormat(compressed.length === pieces.length && compressed.every((chunk, index) => chunk.decodedBytes === pieces[index].length), "codec returned an unexpected chunk partition");
  const container: DsavContainer = { header, namesOffset, nodesOffset, payloadBytes: payload.length, chunks: compressed };
  // The rebuilt directory must parse as a valid document before anything is written.
  parseDsavPayload(container, payload);
  const encoded = encodeDsavContainer(container);
  const readback = await decodeDsav(encoded, codec);
  requireFormat(readback.payload.equals(payload), "re-encoded save changed decoded bytes");
  return encoded;
}
