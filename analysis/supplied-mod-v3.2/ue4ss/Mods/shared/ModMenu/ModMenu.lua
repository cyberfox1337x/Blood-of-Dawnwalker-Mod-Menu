...
function M.StyleText(textBlock, size, color)
    ...
    local actualSize = size or defaults.fontItem
    pcall(function()
        if textBlock.Font then
            textBlock.Font.Size = actualSize
        end
    ...
    pcall(function()
        if textBlock.Font then
            textBlock.Font.TypefaceFontName = FName(actualSize > defaults.fontItem and "Bold" or "Regular")
        end
    end)
end
...
function M.CreateLabeledToggle(outer, namePrefix, caption, initialChecked, fontSize)
    ...
    local pill = M.Construct("/Script/UMG.Border", check, namePrefix .. "_Pill")
    local label = M.Construct("/Script/UMG.TextBlock", pill, namePrefix .. "_Label")
    M.StyleText(label, fontSize or defaults.fontItem)
    M.SetLabelText(label, caption)
    pcall(function()
        pill:SetBrushColor(initialChecked and Colors().buttonBgActive or Colors().buttonBg)
        pill:SetPadding({ Left = 12, Top = 7, Right = 12, Bottom = 7 })
        pill:SetContent(label)
        check:SetContent(pill)
        check:SetIsChecked(initialChecked and true or false)
    end)
    return check, label, pill
end
...
function M.CreateTextButton(outer, namePrefix, caption, bgColor, textColor, fontSize)
    ...
    pcall(function()
        ...
        if button.SetPadding then
            button:SetPadding({ Left = 12, Top = 7, Right = 12, Bottom = 7 })
        end
...
function M.New()
    return {
        ...
        headerImagePath = nil, -- optional local PNG loaded through KismetRenderingLibrary
        headerImageHeight = 56,
        kawaiiCards = false, -- opt-in bordered section cards
        cardBg = nil,
        cardColors = nil,
...
function M.ApplyInit(config, opts, ctx)
    ...
    if opts.headerImagePath ~= nil then config.headerImagePath = tostring(opts.headerImagePath) end
    if opts.headerImageHeight ~= nil then
        if type(opts.headerImageHeight) ~= "number" or opts.headerImageHeight < 1 then
            error("ModMenu.Init: headerImageHeight must be a positive number")
        end
        config.headerImageHeight = opts.headerImageHeight
    end
    if opts.kawaiiCards ~= nil then config.kawaiiCards = opts.kawaiiCards == true end
    if opts.cardBg ~= nil then config.cardBg = opts.cardBg end
    if opts.cardColors ~= nil then config.cardColors = opts.cardColors end
...
local function StylePill(ctrl, isOn, ctx)
    local colors = (ctx.config and ctx.config.colors) or {}
    if ctrl.buttonMode then
        pcall(function()
            ctrl.widget:SetBackgroundColor(isOn and (colors.buttonBgActive or colors.buttonBg)
                or colors.buttonBg)
        end)
        ctx.umg.StyleText(ctrl.label, ctx.config.fontItem,
            isOn and (colors.buttonTextActive or colors.buttonText) or colors.buttonText)
        return
    end
    if ctrl.pill == nil then return end
    pcall(function()
        ctrl.pill:SetBrushColor(isOn and (colors.buttonBgActive or colors.buttonBg)
            or colors.buttonBg)
    end)
end
...
function Checkbox.build(ctx)
    ...
    local check, label, pill, buttonMode
    if ctx.config.kawaiiCards then
        check, label = umg.CreateTextButton(
            ctx.contentBox, ctx.namePrefix, Caption(item, current),
            current and ctx.config.colors.buttonBgActive or ctx.config.colors.buttonBg,
            current and ctx.config.colors.buttonTextActive or ctx.config.colors.buttonText,
            ctx.config.fontItem
        )
        buttonMode = true
    else
        check, label, pill = umg.CreateLabeledToggle(
            ctx.contentBox,
            ctx.namePrefix,
...
function Checkbox.apply(ctrl, value, ctx)
    ...
    if not ctrl.buttonMode then
        pcall(function()
            ctrl.widget:SetIsChecked(on)
        end)
    end
    ctx.umg.SetLabelText(ctrl.label, Caption(ctrl.item, on))
    StylePill(ctrl, on, ctx)
end

function Checkbox.pollClick(ctrl, ctx)
    if not ctrl.buttonMode or not ctx.Input.WidgetHovered(ctrl.widget) then
        return false
    end
    return Toggle(ctrl, ctx)
end
...
local function CreatePicker(outer, namePrefix, options, selectedValue, dropOpts, config)
    ...
    -- Runtime-created UMG rows are not virtualized. Hundreds of animated rows
    -- can smear over one another during a fast wheel scroll, so searchable
    -- pickers render a bounded page and rely on their text filter for the rest.
    if searchable then maxVisible = math.min(maxVisible, 48) end
...
-- widgets/meter.lua
package.preload["ModMenu.widgets.meter"] = function(...)
-- Compact read-only stat row with a native UMG progress bar.
local Meter = {}
Meter.type = "meter"
...
local function Parts(value)
    if type(value) == "table" then
        ...
        return math.max(0, math.min(1, percent)), tostring(value.text or string.format("%.0f%%", percent * 100))
    end
    local percent = tonumber(value) or 0
    return math.max(0, math.min(1, percent)), string.format("%.0f%%", percent * 100)
end

function Meter.validate(item, sectionId, index)
    if item.id == nil or item.id == "" then
        error(string.format("Register(%s) items[%d] meter requires .id", tostring(sectionId), index))
    end
    if item.label == nil then
        error(string.format("Register(%s) items[%d] meter requires .label", tostring(sectionId), index))
    end
end

function Meter.seed(sectionId, item, values)
    local key = tostring(sectionId) .. "." .. tostring(item.id)
    if values[key] == nil then values[key] = item.default or 0 end
end

function Meter.build(ctx)
    local valueKey = ctx.ValueKey(ctx.section.id, ctx.item.id)
    local value = ctx.values[valueKey]
    if value == nil then value = ctx.item.default or 0; ctx.values[valueKey] = value end
    local percent, text = Parts(value)
    local root = ctx.umg.Construct("/Script/UMG.VerticalBox", ctx.contentBox, ctx.namePrefix .. "_Meter")
    local row = ctx.umg.Construct("/Script/UMG.HorizontalBox", root, ctx.namePrefix .. "_MeterRow")
    local label = ctx.umg.Construct("/Script/UMG.TextBlock", row, ctx.namePrefix .. "_MeterLabel")
    local amount = ctx.umg.Construct("/Script/UMG.TextBlock", row, ctx.namePrefix .. "_MeterValue")
    ctx.umg.StyleText(label, ctx.config.fontItem, ctx.config.colors.textPrimary)
    ctx.umg.StyleText(amount, ctx.config.fontItem, ctx.item.color or ctx.config.colors.textAccent)
    ctx.umg.SetLabelText(label, ctx.item.label)
    ctx.umg.SetLabelText(amount, text)
    local left = row:AddChildToHorizontalBox(label)
    local right = row:AddChildToHorizontalBox(amount)
    pcall(function()
        left:SetSize({ SizeRule = 1, Value = 1.0 })
        right:SetSize({ SizeRule = 0, Value = 0.0 })
        right:SetHorizontalAlignment(2)
    end)
    root:AddChildToVerticalBox(row)
    local size = ctx.umg.Construct("/Script/UMG.SizeBox", root, ctx.namePrefix .. "_MeterSize")
    local bar = ctx.umg.Construct("/Script/UMG.ProgressBar", size, ctx.namePrefix .. "_MeterBar")
    pcall(function()
        size:SetHeightOverride(7)
        bar:SetPercent(percent)
        bar:SetFillColorAndOpacity(ctx.item.color or ctx.config.colors.textAccent)
        size:SetContent(bar)
    end)
    local barSlot = root:AddChildToVerticalBox(size)
    pcall(function() barSlot:SetPadding({ Left = 0, Top = 4, Right = 0, Bottom = 2 }) end)
    ctx.umg.AddToContent(ctx, root)
    ctx.umg.AddItemPad(ctx, ctx.namePrefix .. "_Pad", 10)
    table.insert(ctx.liveControls, {kind="meter", sectionId=ctx.section.id, item=ctx.item,
        widget=root, bar=bar, valueLabel=amount, valueKey=valueKey})
end

function Meter.apply(ctrl, value, ctx)
    local percent, text = Parts(value)
    pcall(function() ctrl.bar:SetPercent(percent) end)
    ctx.umg.SetLabelText(ctrl.valueLabel, text)
end
...
register(require("ModMenu.widgets.meter"))
...
function M.New(opts)
    return {
        ...
        headerTexture = nil, ---@type any optional imported decorative PNG
        menuOpen = false,
        contentGen = 0,
        pollHandle = nil,
...
function M.ApplyPercentLayout(slot, config)
    ...
    local minX
    local maxX
    if config.dock == "center" then
        minX = 0.5 - (width / 2)
        maxX = 0.5 + (width / 2)
    elseif config.dock == "left" then
        minX = edge
        maxX = edge + width
    else
        minX = 1.0 - width - edge
...
function M.BuildHeader(S, section, contentBox, suffix)
    ...
    local titleColor = S.config.kawaiiCards and colors.textAccent or colors.textPrimary
    local title = AddHeaderText(row, name .. "_Title", titleText, fontSize, titleColor, true)
...
-- shell/confirm.lua
package.preload["ModMenu.shell.confirm"] = function(...)
--[[
...
local function AddHeaderArt(S, contentBox, suffix)
    local path = S.config.headerImagePath
    if type(path) ~= "string" or path == "" then return end
    if not IsValid(S.headerTexture) then
        local ok, texture = pcall(function()
            local lib = StaticFindObject("/Script/Engine.Default__KismetRenderingLibrary")
            local world = UEHelpers.GetGameInstance()
            if not IsValid(lib) or not IsValid(world) then return nil end
            return lib:ImportFileAsTexture2D(world, path)
        end)
        if ok and IsValid(texture) then
            S.headerTexture = texture
        else
            Log("Kawaii header image unavailable; continuing with native decorations")
            return
        end
    end

    local size = Construct("/Script/UMG.SizeBox", contentBox, "ModMenu_HeaderArtSize_" .. suffix)
    local image = Construct("/Script/UMG.Image", size, "ModMenu_HeaderArt_" .. suffix)
    pcall(function()
        size:SetHeightOverride(S.config.headerImageHeight or 56)
        image:SetBrushFromTexture(S.headerTexture, true)
        size:SetContent(image)
    end)
    local slot = contentBox:AddChildToVerticalBox(size)
    Umg.FillVerticalSlot(slot)
    AddSpacer(contentBox, "ModMenu_HeaderArtPad_" .. suffix, 4)
end
...
function M.BuildContent(S)
    ...
    StyleText(title, config.fontTitle, config.colors.textAccent)
    SetLabelText(title, config.title)
    pcall(function() title:SetJustification(1) end)
    local titleSlot = contentBox:AddChildToVerticalBox(title)
    Umg.FillVerticalSlot(titleSlot)
    ...
    StyleText(hint, config.fontHint, config.colors.textMuted)
    SetLabelText(hint, hintText)
    pcall(function() hint:SetJustification(1) end)
    local hintSlot = contentBox:AddChildToVerticalBox(hint)
    Umg.FillVerticalSlot(hintSlot)

    if not config.kawaiiCards then
        -- Stock shell chrome: the themed layout is deliberately centered.
    ...
    for sIndex, section in ipairs(visible) do
        ...
        local sectionHost = contentBox
        if config.kawaiiCards then
            local accents = config.cardColors
            local accent = config.colors.panelBorder
            if type(accents) == "table" and #accents > 0 then
                accent = accents[((sIndex - 1) % #accents) + 1]
            end
            local cardOutline = Construct(
                "/Script/UMG.Border", contentBox,
                string.format("ModMenu_CardOutline_%s_%s", section.id, suffix)
            )
            local cardFill = Construct(
                "/Script/UMG.Border", cardOutline,
                string.format("ModMenu_CardFill_%s_%s", section.id, suffix)
            )
            local cardBox = Construct(
                "/Script/UMG.VerticalBox", cardFill,
                string.format("ModMenu_CardBox_%s_%s", section.id, suffix)
            )
            pcall(function()
                cardOutline:SetBrushColor(accent)
                cardOutline:SetPadding({ Left = 1, Top = 1, Right = 1, Bottom = 1 })
                cardFill:SetBrushColor(config.cardBg or config.colors.confirmCardBg)
                cardFill:SetPadding({ Left = 14, Top = 10, Right = 14, Bottom = 14 })
                cardFill:SetContent(cardBox)
                cardOutline:SetContent(cardFill)
            end)
            local cardSlot = contentBox:AddChildToVerticalBox(cardOutline)
            Umg.FillVerticalSlot(cardSlot)
            sectionHost = cardBox
        end

        local itemParent = sectionHost
        if Collapse.IsCollapsible(section) then
            local header = Collapse.BuildHeader(S, section, sectionHost, suffix)
            local body = Construct(
                "/Script/UMG.VerticalBox",
                sectionHost,
                string.format("ModMenu_SecBody_%s_%s", section.id, suffix)
            )
            sectionHost:AddChildToVerticalBox(body)
            ...
            StyleText(secTitle, config.fontSection, config.colors.textAccent)
            SetLabelText(secTitle, section.title or section.id)
            pcall(function() secTitle:SetJustification(0) end)
            sectionHost:AddChildToVerticalBox(secTitle)
        end
        AddSpacer(itemParent, string.format("ModMenu_SecPad_%s_%s", section.id, suffix), 8)

        -- Kawaii layout: quiet information column on the left, clearly aligned
        -- actions on the right. This mirrors an actual game settings panel instead
        -- of stacking every debug control into one long receipt.
        local infoBox, actionBox = nil, nil
        if config.kawaiiCards then
            local infoCount, actionCount = 0, 0
            for _, item in ipairs(section.items) do
                if item.type == "label" or item.type == "separator" or item.type == "meter" then
                    infoCount = infoCount + 1
                else
                    actionCount = actionCount + 1
                end
            end
            if infoCount > 0 and actionCount > 0 then
                local columns = Construct(
                    "/Script/UMG.HorizontalBox", itemParent,
                    string.format("ModMenu_CardColumns_%s_%s", section.id, suffix)
                )
                infoBox = Construct(
                    "/Script/UMG.VerticalBox", columns,
                    string.format("ModMenu_CardInfo_%s_%s", section.id, suffix)
                )
                local dividerSize = Construct(
                    "/Script/UMG.SizeBox", columns,
                    string.format("ModMenu_CardDividerSize_%s_%s", section.id, suffix)
                )
                local divider = Construct(
                    "/Script/UMG.Border", dividerSize,
                    string.format("ModMenu_CardDivider_%s_%s", section.id, suffix)
                )
                actionBox = Construct(
                    "/Script/UMG.VerticalBox", columns,
                    string.format("ModMenu_CardActions_%s_%s", section.id, suffix)
                )
                pcall(function()
                    dividerSize:SetWidthOverride(1)
                    divider:SetBrushColor(config.colors.confirmDivider or config.colors.panelBorder)
                    dividerSize:SetContent(divider)
                end)
                local leftSlot = columns:AddChildToHorizontalBox(infoBox)
                local dividerSlot = columns:AddChildToHorizontalBox(dividerSize)
                local rightSlot = columns:AddChildToHorizontalBox(actionBox)
                pcall(function()
                    leftSlot:SetSize({ SizeRule = 1, Value = 0.46 })
                    leftSlot:SetPadding({ Left = 8, Top = 4, Right = 18, Bottom = 6 })
                    dividerSlot:SetSize({ SizeRule = 0, Value = 0.0 })
                    dividerSlot:SetPadding({ Left = 0, Top = 2, Right = 0, Bottom = 2 })
                    rightSlot:SetSize({ SizeRule = 1, Value = 0.54 })
                    rightSlot:SetPadding({ Left = 18, Top = 4, Right = 8, Bottom = 6 })
                end)
                local columnsSlot = itemParent:AddChildToVerticalBox(columns)
                Umg.FillVerticalSlot(columnsSlot)
            end
        end
        ...
        for i, item in ipairs(section.items) do
            ...
            ctx.contentBox = ((item.type == "label" or item.type == "separator" or item.type == "meter") and infoBox)
                or actionBox or itemParent
            ctx.section = section
            ctx.item = item
        ...
        if sIndex < #visible then
            AddSpacer(contentBox, "ModMenu_Between_" .. section.id .. "_" .. suffix, config.kawaiiCards and 12 or 18)
        end
    end
end
