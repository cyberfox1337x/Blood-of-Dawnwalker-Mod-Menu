...
cyberfox1337x.function_signature("dawnwalker_private_eye_preview_v9_probe_tests")
...
test("Chaos torso and headgear clone original assets on private skeletal parents without simulation", function()
    local probe, deps, state = fixture()
    local cloth = state.add_source("chaos-cloth")
    local receipt = probe.run(deps)
    assert(receipt.ok, receipt.reason or receipt.cleanup_error)
    local clone = state.components[4]
    assert(#receipt.frames == 9 and clone:GetClothAsset() == cloth.asset and clone:GetAttachParent() == state.components[3])
    ...
    assert(receipt.preview_components[4].cloth.pose_fidelity_verified == false)
end)

test("known empty components and gameplay widgets are recorded without private clones", function()
    local probe, deps, state = fixture()
    ...
    local receipt = probe.run(deps)
    assert(receipt.ok, receipt.reason or receipt.cleanup_error)
    assert(#state.components == 5 and #receipt.appearance_report.sources == 5)
end)

test("Chaos missing private parent and invalid geometry scale reject with owned cleanup", function()
    for _, kind in ipairs({ "parent", "scale" }) do
        local probe, deps, state = fixture()
        local cloth = state.add_source("chaos-cloth")
        if kind == "parent" then cloth.attach_parent = object(999, "Unknown.Parent")
        else cloth.record.cloth.geometry_scale = -1 end
        local receipt = probe.run(deps)
        assert(not receipt.ok and #state.exports == 0 and state.released == 1 and receipt.destruction_acknowledged)
    end
end)

test("Chaos simulation reenable after export is diagnosed before later phase", function()
    local probe, deps, state = fixture()
    state.add_source("chaos-cloth")
    state.on_export = function() state.components[4].bEnableSimulation = true end
    local receipt = probe.run(deps)
    assert(not receipt.ok and #state.exports == 1 and state.released == 1)
end)

print("PASS " .. count .. " private appearance V9 test groups")
