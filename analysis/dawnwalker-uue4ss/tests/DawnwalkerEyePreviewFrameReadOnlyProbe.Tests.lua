...
cyberfox1337x.function_signature("dawnwalker_eye_preview_frame_read_only_probe_tests")
...
test("lowercase rotator returns are read without fabricating missing components", function()
    local deps, objects = fixture()
    function objects.capture:K2_GetComponentRotation() return { pitch = 2, yaw = 3, roll = 4 } end
    objects.capture.RelativeRotation = { Pitch = 5, Yaw = 6, Roll = 7 }
    local result = Probe.run(deps).dolls[1].value.capture
    assert(result.rotation.value.yaw == 3 and result.rotation.value.binding == "pitch,yaw,roll")
    assert(result.relative_rotation.value.yaw == 6)
    function objects.capture:K2_GetComponentRotation() return { pitch = 8, Yaw = 9, Roll = 10 } end
    assert(Probe.run(deps).dolls[1].value.capture.rotation.value.binding == "pitch,Yaw,Roll")
    function objects.capture:K2_GetComponentRotation() return { pitch = 2, yaw = 3 } end
    assert(not Probe.run(deps).dolls[1].value.capture.rotation.ok)
end)

test("head landmarks use observed bone names and a fixed match budget", function()
    local deps, objects = fixture()
    function objects.mesh:GetNumBones() return 30 end
    function objects.mesh:GetBoneName(index) return { ToString = function() return index == 0 and "pelvis" or "eye_" .. index end } end
    function objects.mesh:GetSocketLocation(bone) assert(bone:ToString():find("eye_", 1, true)); return { X = 1, Y = 2, Z = 3 } end
    local landmarks = Probe.run(deps).dolls[1].value.head_landmarks.value
    assert(landmarks.bone_count == 30 and #landmarks.matches == 24 and landmarks.omitted_matches == 5)
    assert(landmarks.matches[1].name == "eye_1" and landmarks.matches[1].location.value.z == 3)
end)

print("PASS " .. total .. " eye-preview frame read-only probe test groups")
