...
cyberfox1337x.function_signature("dawnwalker_private_eye_renderer_v10_tests")
...
test("full appearance sweeps stay at transaction boundaries instead of scaling with the private cohort", function()
    ...
    -- private clones and 39 at seven. V10 keeps the sweeps at transaction and export boundaries,
    -- so the count is flat while the narrow receiver guards grow instead.
    ...
    assert(small.inspections == 10 and large.inspections == 10, small.inspections .. " vs " .. large.inspections)
    assert(small.sweeps == 9 and large.sweeps == 9, small.sweeps .. " vs " .. large.sweeps)
    assert(large.guards > small.guards, "narrow receiver guards should absorb the extra clones")
    assert(small.diagnostics.renderer == "v10")
...
print("Passed " .. count .. " private renderer V10 groups")
