...
cyberfox1337x.function_signature("dawnwalker_private_eye_preview_probe_tests")
...
local function fixture()
    ...
    function actor:K2_DestroyActor() self.destroying = true; state.destroy_calls = (state.destroy_calls or 0) + 1 end
    ...
    function gameplay:FinishSpawningActor(created_actor) assert(created_actor == actor); return actor end
    ...
    state.player, state.actor, state.capture, state.target, state.render = player, actor, capture, target, render
...
test("native factory reuse never destroys a preexisting actor or releases a shared target", function()
    for _, scenario in ipairs({ "actor", "target", "asset_target" }) do
        local probe, deps, state = fixture()
        if scenario == "actor" then state.existing_actor = true
        elseif scenario == "target" then state.existing_target = true
        else state.target.name = "TextureRenderTarget2D /Game/Shared.Target" end
        local receipt = probe.run(deps)
