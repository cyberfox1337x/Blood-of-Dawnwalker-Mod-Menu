...
cyberfox1337x.function_signature("dawnwalker_eye_preview_read_only_probe_tests")
...
local function fixture()
    ...
    local capture = object(50, "SceneCaptureComponent2D Doll.Capture", { "/Script/Engine.SceneCaptureComponent2D" })
    ...
    local mesh = object(70, "SkeletalMeshComponent Doll.Head", { "/Script/Engine.SkeletalMeshComponent" })
    ...
    return deps, { world = world, player = player, doll = doll, inventory = inventory, capture = capture,
        target = target, mesh = mesh, light = light }
end
...
test("native doll references, isolated transforms, render target and capture list are observations only", function()
    ...
    assert(doll.meshes.HeadMeshComponent.asset.address == "71" and doll.meshes.HairMeshComponent.unset)
    assert(doll.lights["Key Light Spot"].value.color.r == 1)
end)
...
test("missing head, oversized target and invalid slot count remain explicit unavailable results", function()
    for _, field in ipairs({ "head", "target", "slots" }) do
        ...
        else function objects.mesh:GetNumMaterials() return 33 end end
        local result = Probe.run(deps)
...
print("PASS " .. total .. " eye-preview read-only probe test groups")
