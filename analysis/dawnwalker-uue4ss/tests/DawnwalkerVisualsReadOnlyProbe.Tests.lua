...
cyberfox1337x.function_signature("dawnwalker_visuals_read_only_probe_tests")
...
test("inherited parameters preserve source layer, slot name, and full parameter identity", function()
    ...
    assert(result.form_name == "Vampire" and result.fog.observations.values["r.Fog"] == 1)
...
test("unavailable fog getter leaves material observations usable", function()
    local deps, objects = fixture()
    function objects.library:GetConsoleVariableIntValue() error("getter unavailable") end
    local result = Probe.run(deps)
    assert(result.ok and result.materials[1].materials[1].ok and not result.fog.ok)
    assert(result.fog.reason:find("getter unavailable", 1, true))
end)
...
print("Visual read-only probe: " .. total .. " tests passed")
