...
cyberfox1337x.function_signature("dawnwalker_private_eye_renderer_v11_tests")
...
test("full appearance sweeps stay at transaction boundaries instead of scaling with the private cohort", function()
    ...
    -- inspections with the pose refresh. V11 removes the three update_preview intermediates and
    -- the two read-only eye_settings sweeps via already_verified, leaving four full verifications
    -- (entry, before scalars, before export, after export) plus the pose refresh = five
    -- inspections, still flat while the narrow receiver guards grow instead.
    ...
    assert(small.inspections == 5 and large.inspections == 5, small.inspections .. " vs " .. large.inspections)
    assert(small.sweeps == 4 and large.sweeps == 4, small.sweeps .. " vs " .. large.sweeps)
    assert(large.guards > small.guards, "narrow receiver guards should absorb the extra clones")
    assert(small.diagnostics.renderer == "v11")
    assert(small.diagnostics.stages.update_preview.calls == 1 and small.diagnostics.stages.capture_preview.calls == 1)
    -- The four full verifications live exactly at the entry, pre-scalar, pre-export and post-export
    -- sites; validate_readback's pinned cohort guard plus scalar pass performs none.
    assert(small.diagnostics.stages.update_preview.full_verifications == 2
...
test("read-only scalar passes never re-run the appearance sweep", function()
    ...
    -- update_preview: entry + pre-scalar fulls and the pose refresh; capture_preview: two export
    -- fulls. The two read-only eye_settings passes add nothing.
    assert(inspections == 5, "expected exactly five inspections, saw " .. inspections)
...
print("Passed " .. count .. " private renderer V11 groups")
