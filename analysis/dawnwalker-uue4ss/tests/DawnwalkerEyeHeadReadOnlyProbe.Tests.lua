...
cyberfox1337x.function_signature("dawnwalker_eye_head_material_read_only_probe_tests")
...
test("inherited parameters preserve source layer, slot name, and full parameter identity", function()
    ...
    local tint = slot.effective_parameters[3]
    assert(tint.name == "ObservedTint" and tint.association == 0 and tint.index == -1 and tint.observed_override.g == 0.4)
    assert(slot.chain[2].overrides == nil)
    assert(result.form_name == "Vampire" and result.fog == nil and result.scope == "eye-head-material-discovery")
...
test("native getter failure keeps source observations and a per-parameter error", function()
    ...
    local slot = Probe.run(deps).materials[1].materials[1]
    assert(slot.ok and slot.effective_parameters[3].observed_override.r == 0.3)
...
test("non-head equipment material methods are never invoked", function()
    local deps, objects = fixture()
    local equipment = object(501, "SkeletalMeshComponent Player.Equipment", { "/Script/Engine.SkeletalMeshComponent" })
    function equipment:GetNumMaterials() error("equipment materials read") end
    function equipment:GetSkeletalMeshAsset() error("equipment asset read") end
    function equipment:GetOwner() error("equipment owner read") end
    function objects.player:K2_GetComponentsByClass() return { equipment, objects.mesh } end
    local result = Probe.run(deps)
    assert(result.ok and #result.materials == 1 and result.materials[1].address == objects.mesh:GetAddress() .. "")
end)

test("a HeadMesh absent from the owned component list never supplies a target", function()
    local deps, objects = fixture()
    function objects.player:K2_GetComponentsByClass() return {} end
    local result = Probe.run(deps)
    assert(not result.ok and result.materials == nil and result.reason:find("exactly once", 1, true))
end)
print("Eye head material read-only probe: " .. total .. " tests passed")
