...
package.loaded.DawnwalkerEyeSessionPilot = { is_active = function() return session_active end,
...
test("variant observation invokes only its read-only module", function()
    ...
    assert(result.ok and result.schema == 8 and #calls == 1 and calls[1].name == "variants")
...
test("human iris uses only the owned inventory phase adapter and native scalar pilot", function()
    ...
    local result = Dispatch.run(input)
    assert(result.ok and result.schema == 8 and #calls == 1 and calls[1].name == "human-iris")
...
test("an active host exclusively owns native operations until explicit stop", function()
    session_active = true
    for _, operation in ipairs({ "observe", "export", "private-instance-roundtrip", "color-roundtrip", "camera-roundtrip",
        "variant-observe", "private-preview-roundtrip", "human-iris-pair-roundtrip", "session-start" }) do
        calls = {}; local result = Dispatch.run(deps(operation))
        assert(not result.ok and #calls == 0 and result.schema == 8)
    end
    calls = {}; assert(Dispatch.run(deps("session-stop")).ok and #calls == 1 and calls[1].name == "session")
    session_active = false
end)
test("session operations only call their inert operation adapter", function()
    for _, operation in ipairs({ "session-start", "session-stop" }) do
        calls = {}; assert(Dispatch.run(deps(operation)).ok and #calls == 1 and calls[1].name == "session")
    end
end)
