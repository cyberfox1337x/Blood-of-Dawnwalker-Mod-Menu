...
cyberfox1337x.function_signature("dawnwalker_eye_preview_geometry_v8_tests")
...
test("observed V7 head and eye dimensions tighten shoulder framing while keeping full-head orbit clearance", function()
    local native = input()
    native.head_radius = 29.122647073438
    native.eye_left.Y, native.eye_right.Y = 20 - 6.5225450970387 / 2, 20 + 6.5225450970387 / 2
    local geometry = Geometry.build_views(native)
    local profile = geometry.profiles["head-and-shoulders"]
    assert(profile.default_distance > 100 and profile.default_distance < 120)
    assert(math.abs(profile.pivot.Z - (197 - geometry.eye_distance * 1.2)) < 0.000001)
    for _, view in ipairs(geometry.views) do
        local center = geometry.head_center
        local distance = math.sqrt((view.location.X - center.X)^2 + (view.location.Y - center.Y)^2 + (view.location.Z - center.Z)^2)
        assert(distance > geometry.head_radius + view.near_clip)
