...
import { MAX_DSAV_CHUNK_BYTES } from "./dsavContainer.js";
...
function serializeChunks(chunks) {
    if (!chunks.length || chunks.length > 4096)
        throw new Error("Invalid codec chunk count.");
    const header = Buffer.alloc(4);
    header.writeUInt32LE(chunks.length);
    const parts = [header];
    let total = 4;
    for (const chunk of chunks) {
        if (chunk.encoded.length < 1 || chunk.encoded.length > MAX_ENCODED_CHUNK_BYTES || !Number.isInteger(chunk.decodedBytes) || chunk.decodedBytes < 1 || chunk.decodedBytes > MAX_DSAV_CHUNK_BYTES)
            throw new Error("Invalid codec chunk size.");
        total += 8 + chunk.encoded.length;
        if (total > MAX_BATCH_BYTES)
            throw new Error("Codec input exceeds the size limit.");
...
function parseResults(bytes, count) {
    if (bytes.length < 4 || bytes.readUInt32LE(0) !== count)
        throw new Error("Codec returned an invalid chunk inventory.");
    const results = [];
    let cursor = 4;
    for (let index = 0; index < count; index++) {
        if (cursor + 4 > bytes.length)
            throw new Error("Codec returned truncated output.");
        const size = bytes.readUInt32LE(cursor);
        cursor += 4;
        if (size < 1 || size > MAX_ENCODED_CHUNK_BYTES || cursor + size > bytes.length)
            throw new Error("Codec returned an invalid output length.");
        results.push(bytes.subarray(cursor, cursor + size));
        cursor += size;
    }
    if (cursor !== bytes.length)
        throw new Error("Codec returned unexpected trailing output.");
    return results;
}
async function verifiedCodecPath(codecPath) {
    if (process.platform !== "win32" || process.arch !== "x64")
        throw new Error("Save decoding requires 64-bit Windows.");
    const absolute = resolve(codecPath);
    const status = await lstat(absolute);
    if (!status.isFile() || status.isSymbolicLink() || status.size > 16 * 1024 * 1024)
        throw new Error("Installed save codec is not a supported ordinary file.");
    const hash = createHash("sha256").update(await readFile(absolute)).digest("hex");
    if (hash !== CODEC_SHA256)
        throw new Error("The installed save codec version has not been verified.");
    return absolute;
}
async function transformChunks(codecPath, mode, chunks) {
    ...
    return new Promise((resolveResult, reject) => {
        ...
        child.stdout.on("data", (part) => {
            ...
            if (outputBytes > MAX_BATCH_BYTES) {
                fail(new Error("Save codec output exceeded its size limit."));
                return;
            }
            output.push(part);
        });
        child.stderr.on("data", (part) => { errorText = (errorText + part.toString("utf8")).slice(0, 2000); });
        child.on("close", code => {
            clearTimeout(timeout);
            if (failed)
                return;
            if (code !== 0) {
                reject(new Error(`Save codec failed. ${errorText.trim() || "No complete output was returned."}`));
                return;
            }
            try {
                resolveResult(parseResults(Buffer.concat(output), chunks.length));
            }
            catch (error) {
                reject(error);
            }
...
export function createInstalledOodleCodec(codecPath = DEFAULT_CODEC) {
    return Object.freeze({
        decompress: (chunks) => transformChunks(codecPath, "decode", chunks),
