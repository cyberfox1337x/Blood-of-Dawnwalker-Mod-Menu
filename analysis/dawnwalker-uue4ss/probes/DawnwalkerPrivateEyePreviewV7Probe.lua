...
cyberfox1337x.function_signature("dawnwalker_private_eye_preview_v7_probe")
...
local DIAGNOSTICS = {
    { phase = "diag-world-ldr", isolated = false, manual = false, intensity = 2000 },
    { phase = "diag-isolated-ldr", isolated = true, manual = false, intensity = 2000 },
    { phase = "diag-manual-zero", isolated = true, manual = true, intensity = 0 },
    { phase = "diag-manual-low", isolated = true, manual = true, intensity = 20 },
    { phase = "diag-manual-high", isolated = true, manual = true, intensity = 200 },
    { phase = "diag-inventory-color", isolated = true, manual = true, intensity = 20, inventory_color = true },
}
...
function Probe.run(deps)
    ...
    local source_snapshot, geometry, math_library, gameplay, spawn, target_outer
    local finish_attempted, spawn_finished, target_new_address = false, false, false
    local frames, diagnostics, view_applied, front_restored = {}, {}, false, false
    local comparison_target, comparison_identity, comparison_outer, comparison_owned, comparison_released
    local current_profile
    local display = { requested_gamma = 2.2, requested_force_linear_gamma = false }
    ...
    local ok, result = pcall(function()
        ...
        for _, profile in ipairs(DIAGNOSTICS) do
            filenames[profile.phase] = "eye-private-" .. deps.nonce:lower() .. "-" .. profile.phase .. ".png"
            require_condition(deps.output_exists(directory .. "/" .. filenames[profile.phase]) == false, "Private diagnostic output already exists")
        end
        ...
        local display_write_ok, display_write_error = pcall(function()
            target.TargetGamma = 2.2
            require_condition(display.force_linear_before.ok and (display.force_linear_before.type == "boolean"
                or display.force_linear_before.type == "number"), "Private target force-linear flag is not a native boolean/byte")
            target.bForceLinearGamma = display.force_linear_before.type == "number" and 0 or false
        end)
        ...
        display.write_ok = display_write_ok
        if not display_write_ok then display.write_error = tostring(display_write_error):sub(1, 256) end
        require_condition(display_write_ok, "Private target display configuration write failed")
        require_condition(display.gamma_after.ok and display.gamma_after.type == "number"
            and math.abs(display.gamma_after.value - 2.2) < 0.00001, "Private target TargetGamma did not read back")
        require_condition(display.force_linear_after.ok and flag(display.force_linear_after.value, false),
            "Private target bForceLinearGamma did not read back")
        ...
        local function export_view(view, profile, output, output_identity, destination)
            ...
            captures = captures + 1
            require_condition(captures <= 15, "Private diagnostic capture budget exceeded")
            exported = true
        ...
        for _, profile in ipairs(DIAGNOSTICS) do
            local output, output_identity = target, target_identity
            if profile.inventory_color then
                verify_capture(target_identity)
                local addresses = existing_addresses(deps, "TextureRenderTarget2D")
                comparison_target = render_library:CreateRenderTarget2D(player:GetWorld(), 1024, 1024, 2, { R = 0, G = 0, B = 0, A = 0 }, false, false)
                comparison_identity = record(comparison_target, "/Script/Engine.TextureRenderTarget2D", "private comparison target")
                require_condition(not addresses[comparison_identity.address] and comparison_identity.address ~= target_identity.address,
                    "Comparison target was not a separately new native resource")
                comparison_owned = true
                comparison_outer = record(comparison_target:GetOuter(), "/Script/Engine.World", "comparison target outer")
                require_condition(same(comparison_target:GetOuter(), world_identity) and comparison_target.SizeX == 1024
                    and comparison_target.SizeY == 1024 and comparison_target.RenderTargetFormat == 2
                    and number(comparison_target.TargetGamma, "comparison target default gamma") == 0,
                    "Comparison target does not match the observed inventory color format/default gamma")
                capture.TextureTarget = comparison_target
                output, output_identity = comparison_target, comparison_identity
            end
            local front = {}
            for key, value in pairs(geometry.views[1]) do front[key] = value end
            front.phase = profile.phase
            export_view(front, profile, output, output_identity, diagnostics)
            if profile.inventory_color then
                verify_capture(comparison_identity)
                capture.TextureTarget = target
                verify_capture(target_identity)
                -- Keep the second owned target detached until the shared finally
                -- cleanup, so a throwing release can never be retried blindly.
            end
        end
        for _, view in ipairs(geometry.views) do
            export_view(view, { phase = "isolated-manual-20cd", isolated = true, manual = true, intensity = 20 }, target, target_identity, frames)
        end
        local sources = {}
        ...
        return { ok = true, schema = 3, kind = "native-private-eye-preview-evidence", boot_id = deps.boot_id, nonce = deps.nonce:lower(),
            frames = frames, diagnostics = diagnostics, width = 1024, height = 1024, render_target_format = 3, capture_source = 2, target_gamma = target.TargetGamma,
            selected_profile = "isolated-manual-20cd", diagnostic_count = 6, capture_limit = 15, render_target_limit = 2,
    ...
    local comparison_release_ok, comparison_release_error = pcall(function()
        if valid(comparison_target) and comparison_owned and not comparison_released then
            require_condition(same(comparison_target, comparison_identity), "Comparison target changed before release")
            require_condition(not valid(capture) or (same(capture, capture_identity) and not valid(capture.TextureTarget)), "Comparison target is still bound")
            render_library:ReleaseRenderTarget2D(comparison_target)
            comparison_released = true
        end
    end)
    ...
    local comparison_receipt = { identity = comparison_identity, outer = comparison_outer, owned = comparison_owned == true,
        new_address = comparison_owned == true, released = comparison_released == true }
    if ok and restore_ok and cleanup_ok and release_ok and comparison_release_ok and source_ok then
        ...
        result.comparison_target = comparison_receipt
    ...
    local failure = { ok = false, created = created, destruction_acknowledged = destroyed, actor_invalidated = actor_invalidated,
        ...
        frames = frames, diagnostics = diagnostics, comparison_target = comparison_receipt, current_profile = current_profile,
    ...
    if not comparison_release_ok then failure.comparison_release_error = tostring(comparison_release_error):sub(1, 1024) end
    ...
    if ok and type(result) == "table" then
        ...
        for _, key in ipairs({ "schema", "kind", "boot_id", "nonce", "width", "height", "render_target_format",
            ...
            "capture_timestamp_known", "limitation", "selected_profile", "diagnostic_count", "capture_limit", "render_target_limit" }) do failure[key] = result[key] end
    end
    return failure
end
