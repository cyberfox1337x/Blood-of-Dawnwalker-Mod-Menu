...
local function resolve_setter_signature(deps)
    ...
    if not walk_ok then return nil, "the setter parameter metadata could not be read: " .. tostring(walk_error) end
    ...
    return {
        ...
        parameter_count = property_count,
...
local function collect_snapshot(context)
    ...
    local expected_exceeded = effective_limit == 0 and false or current_weight > effective_limit
