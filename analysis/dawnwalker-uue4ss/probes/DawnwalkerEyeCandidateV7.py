"""Validate and compose a new V7 candidate; never overwrite or install one."""
...
cyberfox1337x("dawnwalker_eye_candidate_v7_composer")
ROOT = Path(__file__).resolve().parents[3]
SOURCE = Path(__file__).resolve().parent
TESTS = SOURCE.parent / "tests"
FROZEN = ROOT / "qa/eye-appearance/native-pilot-candidate-v6"
DESTINATION = ROOT / "qa/eye-appearance/native-pilot-candidate-v7"
PREVIOUS_MANIFEST = "F07AD63842FAF54A6159948E2564B55227E5EB3E8667B04F07CDB13B73DDC5A3"
PREVIOUS_ISSUER = "643D912F792ACA2ACA0B65480DB51ED93D5965FE7D1AC27641A05D4A388178DB"
SCRIPTS = "Mods/DawnwalkerRequestedReadOnly/Scripts/"
LUA_BIN = Path("C:/Users/Cyberfox1337/AppData/Local/Programs/Lua/bin")
REPLACEMENTS = {
    "main.lua": "DawnwalkerEyeNativeBootstrapV7.lua",
    "ReadOnlyDriver.lua": "DawnwalkerEyeNativeDriverV7.lua",
    "EyeDiscoveryProbe.lua": "DawnwalkerEyeNativeDispatchV7.lua",
    "DawnwalkerPrivateEyePreviewProbe.lua": "DawnwalkerPrivateEyePreviewV7Probe.lua",
}
PROFILE = {
    "maximumExports": 15, "maximumOwnedRenderTargets": 2,
    "diagnosticPhases": ["diag-world-ldr", "diag-isolated-ldr", "diag-manual-zero",
                         "diag-manual-low", "diag-manual-high", "diag-inventory-color"],
    "viewPhases": ["front", "yaw-min", "yaw-max", "eyes-closeup", "head-zoom-min",
                   "head-zoom-max", "eyes-zoom-min", "eyes-zoom-max", "restored-front"],
    "manualExposureCandela": [0, 20, 200], "selectedViewExposureCandela": 20,
    "ldrCaptureSource": 2, "ldrRenderTargetFormat": 3,
    "inventoryCaptureSource": 9, "inventoryRenderTargetFormat": 2, "maximumDimension": 1024,
}
...
SUITES = {
    ...
    "privatePreviewGroups": ("DawnwalkerPrivateEyePreviewV7Probe", "DawnwalkerPrivateEyePreviewProbe"),
    "geometryGroups": ("DawnwalkerEyePreviewGeometry", "DawnwalkerEyePreviewGeometry"),
    "driverGroups": ("DawnwalkerEyeNativeDriverV7", "ReadOnlyDriver"),
    "dispatchGroups": ("DawnwalkerEyeNativeDispatchV7", "EyeDiscoveryProbe"),
    ...
    "bootstrapGroups": ("DawnwalkerEyeNativeBootstrapV7", "main"),
}
...
def validate(stage: Path) -> dict:
    ...
    if len(syntax) != 11:
        raise ValueError("Expected exactly eleven staged Lua payloads")
    records = []
...
def main() -> None:
    ...
    if DESTINATION.exists():
        raise ValueError("V7 destination already exists; composer never overwrites a candidate")
    no_links(FROZEN / "manifest.json")
    if sha256(FROZEN / "manifest.json") != PREVIOUS_MANIFEST:
        raise ValueError("Frozen V6 manifest changed")
    no_links(FROZEN / "New-EyePilotAttestation.ps1")
    if sha256(FROZEN / "New-EyePilotAttestation.ps1") != PREVIOUS_ISSUER:
        raise ValueError("Frozen V6 issuer changed")
    manifest = json.loads((FROZEN / "manifest.json").read_text(encoding="utf-8-sig"))
    if manifest["schema"] != 5 or manifest["revision"] != 6 or len(manifest["payload"]) != 13:
        raise ValueError("Frozen V6 profile differs")
    inputs = []
    for record in manifest["payload"]:
        ...
        if sha256(previous) != record["sha256"] or previous.stat().st_size != record["bytes"]:
            raise ValueError("Frozen V6 payload changed: " + record["path"])
    ...
    with tempfile.TemporaryDirectory(prefix="dawnwalker-eye-v7-") as temporary:
        stage = Path(temporary)
        if stage.resolve().parent != Path(tempfile.gettempdir()).resolve() or not stage.name.startswith("dawnwalker-eye-v7-"):
        ...
        old_schema = "$manifest.schema -ne 5 -or $manifest.revision -ne 6"
        if issuer.count(old_schema) != 1 or issuer.count("'schema=5'") != 1:
            raise ValueError("Frozen issuer schema guard differs")
        issuer = issuer.replace(old_schema, "$manifest.schema -ne 6 -or $manifest.revision -ne 7")
        issuer = issuer.replace("'schema=5'", "'schema=6'")
        ...
        shutil.copyfile(ROOT / "qa/eye-appearance/V7-NATIVE-PILOT.md", stage / "README.md")
        manifest.update(schema=6, revision=7, preparedAtUtc=datetime.now(timezone.utc).isoformat(), privatePreviewProfile=PROFILE)
        manifest["privatePreviewLimits"]["maxFramesPerAttempt"] = 15
