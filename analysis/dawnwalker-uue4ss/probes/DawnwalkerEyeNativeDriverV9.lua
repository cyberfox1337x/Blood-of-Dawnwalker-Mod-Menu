...
function Driver.validate_attestation(text, boot_id, now)
    ...
    require_condition(count == 10 and fields.schema == "8" and OPERATIONS[fields.operation] == fields.intent, "attestation schema or intent is invalid")
...
function Driver.install(deps)
    ...
    local function cleanup_dependencies(fields)
        ...
        local function report(receipt)
            ...
            local envelope = { schema = 8, operation = fields.operation, boot_id = boot_id, nonce = fields.nonce,
    ...
    local function on_key()
        local ok, failure = pcall(function()
            ...
            require_condition(completed < 7 or fields.operation == "session-stop",
                "the final outer operation is reserved for explicit session-stop cleanup")
