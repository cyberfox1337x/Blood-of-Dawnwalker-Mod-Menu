...
cyberfox1337x.function_signature("dawnwalker_private_eye_renderer_v13")
...
function Renderer.new(deps)
    ...
    function adapters.update_preview(candidate, desired)
        ...
        -- V12 re-checked the whole cohort here before the light loop. That is provably redundant:
        -- nothing writes between the mesh loop's last write and the light loop's own first
        -- verify_receiver call, and verify_receiver already re-checks the full cohort (both meshes
        -- and lights) before every write it guards. The light loop's own entry check re-establishes
        -- the identical invariant with zero intervening state change, so the standalone call here
        -- added a second cohort scan for no additional safety. Removed in V13.
        ...
        -- V12 also re-checked the cohort here before eye_settings. Also redundant by the same
        -- argument: eye_settings' write pass unconditionally runs verify_receiver before its very
        -- first scalar write (the already_verified flag only skips its own upfront full sweep, not
        -- that per-write guard), and a canonical human settings payload always has eight consumed
        -- values, so that first write-path check is guaranteed to run before any scalar is set.
        -- Removed in V13. eye_settings may therefore skip its own sweep via already_verified; any
        -- drift is still caught by capture_preview's before-export full sweep before evidence is
        -- exported, and by every remaining per-write receiver guard in between.
        local settings = eye_settings(deps, candidate, desired.settings, true, true)
        -- This cohort check, immediately before apply_view, stays: apply_view writes the private
        -- camera transform directly and does not self-guard, so this is the only check protecting
        -- that write.
        ...
        -- V12 also re-checked the cohort here, after apply_view. Also redundant: DawnwalkerEyePreviewSession.lua
        -- calls update_preview and validate_readback back-to-back with no intervening native write
        -- or engine-tick boundary, and validate_readback opens with its own verify_private_cohort
        -- call that re-establishes the identical invariant. Removed in V13.
