"""Validate and compose a new V10 candidate; never overwrite or install one.

V10 differs from the frozen V9 candidate in exactly one payload: the private preview renderer,
which moves from DawnwalkerPrivateEyeRendererV9.lua to DawnwalkerPrivateEyeRendererV10.lua. The
...
Because the profile is unchanged the frozen V9 issuer is reused verbatim rather than rewritten.
"""
...
cyberfox1337x("dawnwalker_eye_candidate_v10_composer")
ROOT = Path(__file__).resolve().parents[3]
SOURCE = Path(__file__).resolve().parent
TESTS = SOURCE.parent / "tests"
FROZEN = ROOT / "qa/eye-appearance/native-pilot-candidate-v9"
DESTINATION = ROOT / "qa/eye-appearance/native-pilot-candidate-v10"
PREVIOUS_MANIFEST = "7C1D38959E5E50918492EA402BC2FEAF80AAF16AFE26AB38B934F742000FE43C"
...
README_SOURCE = ROOT / "qa/eye-appearance/V10-SESSION-PILOT.md"
# The only staged leaf whose source changes. Every other payload is reused byte-identically
# from the frozen V9 candidate after its recorded hash is re-verified.
REPLACEMENTS = {"DawnwalkerPrivateEyeRenderer.lua": "DawnwalkerPrivateEyeRendererV10.lua"}
RENDERER_PROVENANCE = "analysis/dawnwalker-uue4ss/probes/DawnwalkerPrivateEyeRendererV10.lua"
...
SUITES = {
    ...
    "privateRendererGroups": ("DawnwalkerPrivateEyeRendererV10", "DawnwalkerPrivateEyeRenderer"),
...
def verify_issuer(issuer: str) -> None:
    ...
    if issuer.count(guard) != 1 or issuer.count("'schema=8'") != 1:
        raise ValueError("Frozen V9 issuer schema guard differs; V10 reuses it unchanged")
    for anchor in ("'session-start' = 'eye-session-start'", "'session-stop' = 'eye-session-stop'",
        ...
        if issuer.count(anchor) != 1:
            raise ValueError("Frozen V9 issuer session or payload allowlist differs: " + anchor)


def main() -> None:
    no_links(DESTINATION)
    if DESTINATION.exists():
        raise ValueError("V10 destination already exists; composer never overwrites a candidate")
    no_links(FROZEN / "manifest.json")
    if sha256(FROZEN / "manifest.json") != PREVIOUS_MANIFEST:
        raise ValueError("Frozen V9 manifest changed")
    no_links(FROZEN / "New-EyePilotAttestation.ps1")
    if sha256(FROZEN / "New-EyePilotAttestation.ps1") != PREVIOUS_ISSUER:
        raise ValueError("Frozen V9 issuer changed")
    ...
    if manifest["schema"] != 8 or manifest["revision"] != 9 or len(manifest["payload"]) != EXPECTED_PAYLOADS:
        raise ValueError("Frozen V9 profile differs")
    replaced = 0
    inputs = []
    for record in manifest["payload"]:
        ...
        if sha256(previous) != record["sha256"] or previous.stat().st_size != record["bytes"]:
            raise ValueError("Frozen V9 payload changed: " + record["path"])
        ...
        if replacement:
            ...
            if sha256(selected) == record["sha256"]:
                raise ValueError("The V10 renderer payload is identical to V9; there is nothing to compose")
        inputs.append((record["path"], selected, sha256(selected)))
    if replaced != len(REPLACEMENTS):
        raise ValueError("The frozen V9 manifest does not carry every replaced payload")
    # Validate an isolated temporary copy before exposing a candidate directory.
    with tempfile.TemporaryDirectory(prefix="dawnwalker-eye-v10-") as temporary:
        stage = Path(temporary)
        if stage.resolve().parent != Path(tempfile.gettempdir()).resolve() or not stage.name.startswith("dawnwalker-eye-v10-"):
        ...
        # Schema, revision and every bounded profile stay exactly as V9 recorded them.
