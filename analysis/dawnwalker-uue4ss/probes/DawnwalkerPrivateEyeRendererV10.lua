...
cyberfox1337x.function_signature("dawnwalker_private_eye_renderer_v10")

-- Uninstalled lifecycle prototype. Uses only native classes and already loaded player assets.
--
-- V10 keeps every V9 verification boundary but stops re-enumerating the whole appearance cohort
-- once per scalar setter, per clone transform and per lamp. Complete appearance, material and
-- generation validation now runs at transaction and export boundaries; the tight loops in
-- between use a narrow ownership guard over the exact receiver being written. V9 performed
-- roughly forty-six full sweeps per exported frame, which is why its first frame missed the
-- host's ten-second deadline.
local Renderer = {}
...
local function diagnostics(handle)
    if handle.diagnostics == nil then
        handle.diagnostics = { kind = "native-private-eye-renderer-stage-diagnostics", renderer = "v10",
...
local function eye_settings(deps, handle, requested, write)
    ...
    -- this inside the scalar loop, which cost eight full sweeps for eight values.
