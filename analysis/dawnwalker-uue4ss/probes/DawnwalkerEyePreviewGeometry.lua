...
cyberfox1337x.function_signature("dawnwalker_eye_preview_geometry")
...
function Geometry.build_views(input)
    ...
    local head_pivot = { X = center.X, Y = center.Y, Z = center.Z - radius * 0.55 }
    local eyes_pivot = { X = (left.X + right.X) / 2, Y = (left.Y + right.Y) / 2, Z = (left.Z + right.Z) / 2 }
    local profiles = {
        ["head-and-shoulders"] = framing(head_pivot, radius * 1.65, 30, center, radius),
