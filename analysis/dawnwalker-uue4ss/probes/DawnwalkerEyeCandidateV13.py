"""Validate and compose a new V13 candidate; never overwrite or install one.

V13 differs from the frozen V12 candidate in exactly one payload: the private preview renderer,
which moves from DawnwalkerPrivateEyeRendererV12.lua to DawnwalkerPrivateEyeRendererV13.lua. The
...
Because the profile is unchanged the frozen V12 issuer (itself the V9/V10 issuer, reused verbatim)
...
cyberfox1337x("dawnwalker_eye_candidate_v13_composer")
ROOT = Path(__file__).resolve().parents[3]
SOURCE = Path(__file__).resolve().parent
TESTS = SOURCE.parent / "tests"
FROZEN = ROOT / "qa/eye-appearance/native-pilot-candidate-v12"
DESTINATION = ROOT / "qa/eye-appearance/native-pilot-candidate-v13"
PREVIOUS_MANIFEST = "B95C21B005BDC631D89B4389F40A9E7039C26109141E4774FA1D1F36723A7C9A"
...
README_SOURCE = ROOT / "qa/eye-appearance/V13-SESSION-PILOT.md"
# The only staged leaf whose source changes. Every other payload is reused byte-identically
# from the frozen V12 candidate after its recorded hash is re-verified.
REPLACEMENTS = {"DawnwalkerPrivateEyeRenderer.lua": "DawnwalkerPrivateEyeRendererV13.lua"}
RENDERER_PROVENANCE = "analysis/dawnwalker-uue4ss/probes/DawnwalkerPrivateEyeRendererV13.lua"
...
SUITES = {
    ...
    "privateRendererGroups": ("DawnwalkerPrivateEyeRendererV13", "DawnwalkerPrivateEyeRenderer"),
...
def verify_issuer(issuer: str) -> None:
    ...
    if issuer.count(guard) != 1 or issuer.count("'schema=8'") != 1:
        raise ValueError("Frozen V12 issuer schema guard differs; V13 reuses it unchanged")
    for anchor in ("'session-start' = 'eye-session-start'", "'session-stop' = 'eye-session-stop'",
        ...
        if issuer.count(anchor) != 1:
            raise ValueError("Frozen V12 issuer session or payload allowlist differs: " + anchor)


def main() -> None:
    no_links(DESTINATION)
    if DESTINATION.exists():
        raise ValueError("V13 destination already exists; composer never overwrites a candidate")
    no_links(FROZEN / "manifest.json")
    if sha256(FROZEN / "manifest.json") != PREVIOUS_MANIFEST:
        raise ValueError("Frozen V12 manifest changed")
    no_links(FROZEN / "New-EyePilotAttestation.ps1")
    if sha256(FROZEN / "New-EyePilotAttestation.ps1") != PREVIOUS_ISSUER:
        raise ValueError("Frozen V12 issuer changed")
    ...
    if manifest["schema"] != 8 or manifest["revision"] != 9 or len(manifest["payload"]) != EXPECTED_PAYLOADS:
        raise ValueError("Frozen V12 profile differs")
    replaced = 0
    inputs = []
    for record in manifest["payload"]:
        ...
        if sha256(previous) != record["sha256"] or previous.stat().st_size != record["bytes"]:
            raise ValueError("Frozen V12 payload changed: " + record["path"])
        ...
        if replacement:
            ...
            if sha256(selected) == record["sha256"]:
                raise ValueError("The V13 renderer payload is identical to V12; there is nothing to compose")
        inputs.append((record["path"], selected, sha256(selected)))
    if replaced != len(REPLACEMENTS):
        raise ValueError("The frozen V12 manifest does not carry every replaced payload")
    # Validate an isolated temporary copy before exposing a candidate directory.
    with tempfile.TemporaryDirectory(prefix="dawnwalker-eye-v13-") as temporary:
        stage = Path(temporary)
        if stage.resolve().parent != Path(tempfile.gettempdir()).resolve() or not stage.name.startswith("dawnwalker-eye-v13-"):
        ...
        # Schema, revision and every bounded profile stay exactly as V12 recorded them.
