...
cyberfox1337x.function_signature("dawnwalker_appearance_sources")
...
local function inspect(deps, progress)
    ...
    local report = { schema = 1, player = record(player, PLAYER), world = record(player:GetWorld(), ENGINE .. "World"),
    ...
    local function add_actor(actor)
        ...
        for _, component in ipairs(array(actor:K2_GetComponentsByClass(mesh_class), 32)) do
            ...
            if not meshes[component_identity.address] then
                ...
                if component:IsA(ENGINE .. "SkeletalMeshComponent") then
                ...
                if observed.kind ~= "unsupported" and not valid(entry.asset) then
                    observed.unsupported_reason = "Source component has no loaded asset"
                end
                ...
                for slot = 0, count - 1 do
                    ...
                    if not valid(material) then observed.unsupported_reason = "Source material binding is unavailable" end
                end
