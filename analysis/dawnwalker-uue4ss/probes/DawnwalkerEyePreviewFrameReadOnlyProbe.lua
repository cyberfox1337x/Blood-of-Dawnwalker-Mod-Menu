...
cyberfox1337x.function_signature("dawnwalker_eye_preview_frame_read_only_probe")
...
local function rotation(value, label)
    local layouts = { { "pitch", "Yaw", "Roll" }, { "Pitch", "Yaw", "Roll" }, { "pitch", "yaw", "roll" } }
    for _, fields in ipairs(layouts) do
        local ok, result = pcall(function()
            return { pitch = finite_number(value[fields[1]], label .. " pitch"),
                yaw = finite_number(value[fields[2]], label .. " yaw"),
                roll = finite_number(value[fields[3]], label .. " roll"), binding = table.concat(fields, ",") }
        end)
        if ok then return result end
    end
    error(label .. " has no complete finite reflected rotator layout", 0)
end
...
local function capture_record(capture, doll)
    ...
    record.relative_rotation = observe(function() return rotation(capture.RelativeRotation, "capture relative rotation") end)
...
local function head_landmarks(mesh)
    local count = bounded_integer(mesh:GetNumBones(), 1, 2048, "head bone count")
    local result = { bone_count = count, matches = {}, omitted_matches = 0 }
    for index = 0, count - 1 do
        local bone = mesh:GetBoneName(index)
        local name = text(bone:ToString(), "bone name")
        local lower = name:lower()
        if lower:find("head", 1, true) or lower:find("eye", 1, true) or lower:find("neck", 1, true) then
            if #result.matches < 24 then
                result.matches[#result.matches + 1] = { index = index, name = name,
                    location = observe(function() return vector(mesh:GetSocketLocation(bone), "bone location") end) }
            else result.omitted_matches = result.omitted_matches + 1 end
        end
    end
    return result
end
...
local function doll_record(doll, player, world_record, validation)
    ...
    record.head_landmarks = observe(function() return head_landmarks(doll.HeadMeshComponent) end)
