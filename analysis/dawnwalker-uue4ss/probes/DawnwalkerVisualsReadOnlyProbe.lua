...
cyberfox1337x.function_signature("dawnwalker_visuals_read_only_probe")
...
local function local_parameters(material, observed, depth)
    ...
    for _, entry in ipairs(array_entries(material.ScalarParameterValues, 128, "scalar parameters")) do
    ...
    for _, entry in ipairs(array_entries(material.VectorParameterValues, 128, "vector parameters")) do
        local record = parameter_record(entry)
    ...
    for _, entry in ipairs(array_entries(material.TextureParameterValues, 128, "texture parameters")) do
        local record = parameter_record(entry)
...
local function effective_parameters(material, observed)
    local result, seen = {}, {}
...
local function material_chain(material)
    ...
    for depth = 1, 8 do
        ...
        if material:IsA(INSTANCE_CLASS) ~= true then
            ...
            return chain, effective_parameters(leaf, observed)
        end
        record.overrides = local_parameters(material, observed, depth - 1)
...
local function materials(player, deps)
    ...
    require_condition(#components > 0, "player has no skeletal meshes")
    ...
    for _, component in ipairs(components) do
        ...
        for index = 0, count - 1 do
            ...
            local ok, chain, readbacks = pcall(function() return material_chain(material) end)
            entry.ok = ok
            if ok then
...
local function same_object(actual, expected, expected_class, label)
    ...
    require_condition(current.address == expected.address and current.name == expected.name
        and current.unset == expected.unset, label .. " changed during visual probe")
end

local function verify_material_topology(player, deps, records)
    local components = array_entries(player:K2_GetComponentsByClass(deps.static_find_object(MESH_CLASS)), 32, "player meshes after probe")
    require_condition(#components == #records, "player mesh count changed during visual probe")
    local remaining = {}
    ...
    for _, component in ipairs(components) do
        ...
        require_condition(record ~= nil and record.name == identity.name, "player meshes changed during visual probe")
        ...
        require_condition(component:GetNumMaterials() == #record.materials, "material count changed during visual probe")
        local names = array_entries(component:GetMaterialSlotNames(), 32, "material slots after probe")
        require_condition(#names == #record.materials, "material slots changed during visual probe")
        for _, entry in ipairs(record.materials) do
            require_condition(names[entry.slot + 1]:ToString() == entry.slot_name, "material slot name changed during visual probe")
            if entry.ok then
...
local function fog_observations(deps)
    local library = deps.static_find_object("/Script/Engine.Default__KismetSystemLibrary")
    object_record(library, "/Script/Engine.KismetSystemLibrary", "Kismet library", true)
    local values = {}
    for _, name in ipairs({ "r.Fog", "r.VolumetricFog" }) do
        values[name] = number(library:GetConsoleVariableIntValue(name), name)
    end
    return { values = values, limitation = "Getter observations only. A zero can also mean an unavailable variable; visual effect is not verified." }
end

function Probe.run(deps)
    local ok, result = pcall(function()
        require_condition(type(deps) == "table" and type(deps.get_player) == "function"
            and type(deps.static_find_object) == "function", "visual probe dependencies are incomplete")
        require_condition(deps.game_thread == true, "visual probe requires the game thread")
        require_condition(type(deps.identity) == "table" and deps.identity.build_id == BUILD_ID
            and deps.identity.executable_sha256 == EXECUTABLE_SHA256, "visual discovery build attestation mismatch")
        ...
        local snapshot = { ok = true, mutation_authorized = false, gameplay_verified = false,
        ...
        local fog_ok, fog = pcall(function() return fog_observations(deps) end)
        snapshot.fog = fog_ok and { ok = true, observations = fog } or { ok = false, reason = tostring(fog) }
        ...
        require_condition(object_record(current_player, PLAYER_CLASS, "player after probe").address == player_record.address
            ...
            "player, world, or form changed during visual probe")
...
function Probe.format_lines(result)
    ...
    local function visit(prefix, entry, depth)
        require_condition(depth <= 16 and #lines < 200000, "visual report exceeds its bound")
