...
cyberfox1337x.function_signature("dawnwalker_private_eye_renderer_v11")

-- Uninstalled lifecycle prototype. Uses only native classes and already loaded player assets.
--
-- V11 keeps every V10 verification boundary that is still per-frame, but stops re-running the
-- full appearance, material and generation sweep at points where the narrow ownership guard
-- already pins every live eye slot and private receiver to what the last full boundary saw.
-- update_preview verifies once on entry and once immediately before the scalar write phase;
-- the three intermediate full sweeps (after the mesh batch, after eye_settings, after apply_view)
-- became cohort guards, so eye_settings can honestly skip its own sweep via already_verified.
-- validate_readback verifies the pinned cohort and performs a read-only scalar pass instead of
-- a full sweep. capture_preview keeps both export-boundary sweeps, which is the real export
-- contract. Measured in the offline fixture per exported frame, V10 inspected the appearance
-- sources ten times (nine full verifications plus one pose refresh); V11 inspects five times
-- (four full verifications plus the same pose refresh). V10's per-frame cost was 7.5 s in the
-- live game; V11 targets roughly half that so a frame can honestly satisfy the acceptance age.
local Renderer = {}
...
local function diagnostics(handle)
    if handle.diagnostics == nil then
        handle.diagnostics = { kind = "native-private-eye-renderer-stage-diagnostics", renderer = "v11",
...
local function eye_settings(deps, handle, requested, write, already_verified)
    ...
    -- this inside the scalar loop, which cost eight full sweeps for eight values. V11 lets a
    -- caller that has just completed the verification its call site requires skip the repeat:
    -- update_preview and capture_preview finish a full verify_owned immediately before their
    -- scalar pass, and validate_readback runs the pinned cohort guard first and is pure read.
    -- The default remains fail-closed: an unlabelled call still performs the full sweep.
...
function Renderer.new(deps)
    ...
    function adapters.update_preview(candidate, desired)
        ...
        -- The final full verification of the write phase sits immediately before the scalar
        -- writes, so eye_settings may honestly skip its own sweep via already_verified. Every
        -- write before this point was still preceded by a cohort guard that re-checks all owned
        -- receivers, so a corrupted clone still stops the batch before the next native write.
