...
cyberfox1337x.function_signature("dawnwalker_private_eye_preview_v8_probe")
...
local Appearance = require("DawnwalkerAppearanceSources")
...
local function snapshot_sources(deps, record_appearance)
    ...
    for _, source in ipairs(observed.snapshot.sources) do
        ...
        if observed_source.kind == "unsupported" then
...
local function observe_clone(owned, actor_identity, private_by_source)
    ...
    local asset = source.kind == "skeletal" and mesh:GetSkeletalMeshAsset()
        or source.kind == "static" and mesh.StaticMesh or mesh.GroomAsset
...
function Probe.run(deps)
    ...
    local ok, result = pcall(function()
        ...
        local mesh_classes = { skeletal = "/Script/Engine.SkeletalMeshComponent", static = "/Script/Engine.StaticMeshComponent", groom = "/Script/HairStrandsCore.GroomComponent" }
        ...
        for _, kind in ipairs({ "skeletal", "static", "groom" }) do
