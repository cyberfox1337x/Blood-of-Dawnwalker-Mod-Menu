"""Validate and compose a new V12 candidate; never overwrite or install one.

V12 differs from the frozen V11 candidate in exactly one payload: the private preview renderer,
which moves from DawnwalkerPrivateEyeRendererV11.lua to DawnwalkerPrivateEyeRendererV12.lua. The
...
Because the profile is unchanged the frozen V11 issuer (itself the V9/V10 issuer, reused verbatim)
...
cyberfox1337x("dawnwalker_eye_candidate_v12_composer")
ROOT = Path(__file__).resolve().parents[3]
SOURCE = Path(__file__).resolve().parent
TESTS = SOURCE.parent / "tests"
FROZEN = ROOT / "qa/eye-appearance/native-pilot-candidate-v11"
DESTINATION = ROOT / "qa/eye-appearance/native-pilot-candidate-v12"
PREVIOUS_MANIFEST = "7EBA1741517C1A69F3E758CBC73E325E5EC6636F747EBEB6EBD20D4A6D07BC8D"
...
README_SOURCE = ROOT / "qa/eye-appearance/V12-SESSION-PILOT.md"
# The only staged leaf whose source changes. Every other payload is reused byte-identically
# from the frozen V11 candidate after its recorded hash is re-verified.
REPLACEMENTS = {"DawnwalkerPrivateEyeRenderer.lua": "DawnwalkerPrivateEyeRendererV12.lua"}
RENDERER_PROVENANCE = "analysis/dawnwalker-uue4ss/probes/DawnwalkerPrivateEyeRendererV12.lua"
...
SUITES = {
    ...
    "privateRendererGroups": ("DawnwalkerPrivateEyeRendererV12", "DawnwalkerPrivateEyeRenderer"),
...
def verify_issuer(issuer: str) -> None:
    ...
    if issuer.count(guard) != 1 or issuer.count("'schema=8'") != 1:
        raise ValueError("Frozen V11 issuer schema guard differs; V12 reuses it unchanged")
    for anchor in ("'session-start' = 'eye-session-start'", "'session-stop' = 'eye-session-stop'",
        ...
        if issuer.count(anchor) != 1:
            raise ValueError("Frozen V11 issuer session or payload allowlist differs: " + anchor)


def main() -> None:
    no_links(DESTINATION)
    if DESTINATION.exists():
        raise ValueError("V12 destination already exists; composer never overwrites a candidate")
    no_links(FROZEN / "manifest.json")
    if sha256(FROZEN / "manifest.json") != PREVIOUS_MANIFEST:
        raise ValueError("Frozen V11 manifest changed")
    no_links(FROZEN / "New-EyePilotAttestation.ps1")
    if sha256(FROZEN / "New-EyePilotAttestation.ps1") != PREVIOUS_ISSUER:
        raise ValueError("Frozen V11 issuer changed")
    ...
    if manifest["schema"] != 8 or manifest["revision"] != 9 or len(manifest["payload"]) != EXPECTED_PAYLOADS:
        raise ValueError("Frozen V11 profile differs")
    replaced = 0
    inputs = []
    for record in manifest["payload"]:
        ...
        if sha256(previous) != record["sha256"] or previous.stat().st_size != record["bytes"]:
            raise ValueError("Frozen V11 payload changed: " + record["path"])
        ...
        if replacement:
            ...
            if sha256(selected) == record["sha256"]:
                raise ValueError("The V12 renderer payload is identical to V11; there is nothing to compose")
        inputs.append((record["path"], selected, sha256(selected)))
    if replaced != len(REPLACEMENTS):
        raise ValueError("The frozen V11 manifest does not carry every replaced payload")
    # Validate an isolated temporary copy before exposing a candidate directory.
    with tempfile.TemporaryDirectory(prefix="dawnwalker-eye-v12-") as temporary:
        stage = Path(temporary)
        if stage.resolve().parent != Path(tempfile.gettempdir()).resolve() or not stage.name.startswith("dawnwalker-eye-v12-"):
        ...
        # Schema, revision and every bounded profile stay exactly as V11 recorded them.
