...
local SessionPilot = require("DawnwalkerEyeSessionPilot")
local Operations = {}

function Operations.run(deps)
    local result
    if SessionPilot.is_active() and deps.operation ~= "session-stop" then
        result = { ok = false, reason = "The session host owns the runtime; explicitly stop it before one-shot operations" }
    elseif deps.operation == "session-start" or deps.operation == "session-stop" then
        result = SessionPilot.run(deps)
    elseif deps.operation == "observe" then
    ...
    result.schema, result.operation, result.boot_id, result.nonce = 8, deps.operation, deps.boot_id, deps.nonce
