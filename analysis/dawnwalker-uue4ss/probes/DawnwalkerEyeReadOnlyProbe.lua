...
cyberfox1337x.function_signature("dawnwalker_eye_material_read_only_probe")
...
local function material_chain(material, budget)
    ...
    for depth = 1, 8 do
        ...
        if material:IsA(INSTANCE_CLASS) ~= true then
            ...
            return chain, effective_parameters(leaf, observed, budget)
        end
...
function Probe.run(deps)
    local ok, result = pcall(function()
        ...
        local snapshot = { ok = true, schema = 1, scope = "eye-material-discovery", mutation_authorized = false, gameplay_verified = false,
